<?php
namespace vengine\Repositories;
use Intervention\Image\Facades\Image;
use vengine\Services\Utils\ArrayToDot;
use Symfony\Component\HttpFoundation\File\UploadedFile;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Input;

class BaseRepository implements \vengine\Repositories\BaseRepositoryInterface{
...
//                $this->storedObject->$key = $data[$key];
//            }
                $accessGroupUserCollection = \vengine\Models\AccessGroupUser::with([
                  'accessGroups', 'accessGroups.resourceAccess'
                  ])->where("user_id", '=', \Auth::id())->get();

                  // dd($data[0]->accessGroups->resourceAccess);
                $filterArray = [];

                //START FORRACH

                foreach($accessGroupUserCollection as $accessGroupUser)
                {
                  foreach ($accessGroupUser->accessGroups->resourceAccess as $each) {
