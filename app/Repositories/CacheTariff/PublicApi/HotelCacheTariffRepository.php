...
class HotelCacheTariffRepository extends BaseCacheTariffRepository{
	...
	INNER JOIN meal_plans ON meal_plans.tariff_id = cache_tariffs.tariff_id 
        WHERE DATE(date) >= "'.$startDateCarbon.'" AND DATE(date) <= "'.$endDateCarbon.'"  
            AND zone_id IN ('.  implode(",", $zones).') AND hotel_id = '.$hotelID.' 
        GROUP BY room_type_id , meal_type_id
        )temp_room_tariff
...
ORDER BY hotel_id, room_type_id, calculated_total ASC
